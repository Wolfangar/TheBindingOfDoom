            ...
            using (Graphics g = Graphics.FromImage(b))
            {
                for (int x = 0; x < _nodes.Count; x++)
                {
                    for (int y = 0; y < _nodes[x].Count; y++)
                    {
                        ...
                        {
                            g.FillRectangle(new SolidBrush(Color.FromArgb(150, Color.Pink)), sz.Height * x, sz.Width * y, sz.Width, sz.Height);
                        }
                        else if (_nodes[x][y].isFrontier)
                        {
                            g.FillRectangle(new SolidBrush(Color.FromArgb(150, Color.Purple)), sz.Height * x, sz.Width * y, sz.Width, sz.Height);
                        }
                        else if (_nodes[x][y].isStart)
                        {
                            g.FillRectangle(new SolidBrush(Color.FromArgb(255, Color.Blue)), sz.Height * x, sz.Width * y, sz.Width, sz.Height);
                        }
                        else if (_nodes[x][y].parentInfo.Count == 0)
                        {
                            g.FillRectangle(new SolidBrush(Color.FromArgb(255, Color.Black)), sz.Height * x, sz.Width * y, sz.Width, sz.Height);
                        }
                    }
